...
In dieser Variante versuchen wir unsere Zahl bit by bit zu iterieren und nach jeder Iteration die Zahl nach rechts zu schieben, solange unsere Zahl noch nicht Null geworden ist. Dann werden die ersten beiden Bits von  \textit{bm1pi}  \space extrahiert und in den Variablen \textit{powZero} \space und \textit{powOne}\space gespeichert. Diese repräsentieren die Potenzen 0 und 1 der komplexen Zahl.  In jeder Iteration berechnen wir den zugehörigen Potenz von $(-1+i)^{n}$ beginnend von $n=2$ in steigender Reihenfolge. Wenn das aktuelle Bit von \textit{bm1pi} gesetzt (1) ist, werden \textit{baseReal} und \textit{baseImag} zur Summe der Real- und Imaginärteile hinzugefügt.Nachdem alle bits von \textit{bm1pi} durchlaufen wurden, werden die berechneten Werte von \textit{sumReal} und \textit{sumImag} auf die Variablen \textit{real} und \textit{imag} addiert. Diese Variablen sind Zeiger auf Speicher, in dem die Ergebnisse der Berechnung gespeichert werden. Als Optimierung haben wir den bisher gerechneten Potenz von $(-1+i)^{n}$ in einem Variable gespeichert und zur Berechnung des nächsten Potenzes multiplizieren es nur einmal mit $(-1+i)$ statt vom Anfang beginnen, $(-1+i)^{n}$ zu rechnen.

\subsubsection{to\_bm1pi Methode (1.Variante)}
In dieser Methode wollen wir den Algorithmus, den wir in praktischen Teil ausführlich erklärt haben, in C Code implementieren.

Unser Code verwendet eine Schleife, solange entweder alle 128 Bits der Ausgabe berechnet wurden oder beide Reell und Imaginär Teil unserer komplexen Zahl Null werden. Die Variable \textit{result} ist die Ausgabe der Funktion und wird initial auf 0 gesetzt. Die Variable \textit{mask} wird verwendet, um das Bit in \textit{result} zu setzen, entsprechend dem aktuellen Schleifendurchlauf.


In jedem Schleifendurchlauf wird überprüft, ob sowohl der imaginäre als auch der reale Teil des komplexen Werts ungerade sind oder beide gerade sind. Wenn dies der Fall ist, werden die realen und imaginären Teile entsprechend aktualisiert, ohne das Ergebnis zu ändern. Andernfalls werden die realen und imaginären Teile aktualisiert, und das aktuelle Bit in \textit{result} wird auf 1 gesetzt.

Am Ende der Schleife wird das Ergebnis \textit{result} zurückgegeben, das die 128-Bit-BM1PI-Repräsentation des ursprünglichen komplexen Werts darstellt.

\subsubsection{to\_bm1pi Methode (2.Variante)}
